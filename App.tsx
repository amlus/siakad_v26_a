...
const App: React.FC = () => {
  ...
  // Periksa session di localStorage jika ada (simulasi)
  useEffect(() => {
  ...
  return (
    <div className="flex h-screen bg-slate-50">
      <aside className={`bg-white border-r border-slate-200 transition-all duration-500 ease-in-out flex flex-col ${isSidebarOpen ? 'w-64' : 'w-20'}`}>
        {/* Logo Section */}
        <div className={`p-6 flex items-center ${isSidebarOpen ? 'gap-3' : 'justify-center'}`}>
          <div className="min-w-[40px] h-10 bg-emerald-600 rounded-xl flex items-center justify-center text-white font-black text-xl shadow-lg shadow-emerald-500/20 shrink-0">A</div>
          {isSidebarOpen && (
            <div className="overflow-hidden whitespace-nowrap animate-in fade-in duration-500">
              <h1 className="font-black text-slate-800 leading-tight tracking-tight">At Taqwa</h1>
        ...
        {/* Role Switcher Section (Hanya untuk Admin di masa simulasi ini) */}
        {currentUser.role === UserRole.ADMIN && (
          <div className={`p-3 border-t border-slate-100 bg-slate-50/50 ${!isSidebarOpen && 'flex flex-col items-center'}`}>
            <div className={`flex ${isSidebarOpen ? 'flex-col' : 'flex-col items-center'} gap-1.5`}>
               <button onClick={() => switchRole(UserRole.ADMIN)} title="Admin Mode" className={`flex items-center justify-center rounded-xl font-black uppercase tracking-widest transition-all ${isSidebarOpen ? 'w-full py-2.5 text-[10px] bg-emerald-600 text-white' : 'w-10 h-10 text-xs bg-emerald-50 text-emerald-600 border border-emerald-100'}`}>
                  {isSidebarOpen ? 'Administrator' : 'A'}
               </button>
               <button onClick={() => switchRole(UserRole.TUTOR)} title="Tutor Mode" className={`flex items-center justify-center rounded-xl font-black uppercase tracking-widest transition-all ${isSidebarOpen ? 'w-full py-2.5 text-[10px] bg-indigo-600 text-white' : 'w-10 h-10 text-xs bg-indigo-50 text-indigo-600 border border-indigo-100'}`}>
                  {isSidebarOpen ? 'Tutor Mode' : 'T'}
               </button>
               <button onClick={() => switchRole(UserRole.SISWA)} title="Siswa Mode" className={`flex items-center justify-center rounded-xl font-black uppercase tracking-widest transition-all ${isSidebarOpen ? 'w-full py-2.5 text-[10px] bg-blue-600 text-white' : 'w-10 h-10 text-xs bg-blue-50 text-blue-600 border border-blue-100'}`}>
        ...
        {/* User Profile & Logout */}
        <div className={`p-4 border-t border-slate-100 bg-white ${!isSidebarOpen && 'flex flex-col items-center gap-4'}`}>
          <div className={`flex items-center ${isSidebarOpen ? 'gap-3' : 'justify-center'} w-full mb-2`}>
            ...
            {isSidebarOpen && (
              <div className="overflow-hidden animate-in fade-in duration-500 flex-1">
                <p className="text-sm font-black text-slate-800 truncate">{currentUser.name}</p>
                <p className="text-[10px] text-slate-400 font-bold uppercase tracking-tighter leading-none">{currentUser.role}</p>
              </div>
            )}
            {isSidebarOpen && (
              <button 
        ...
        <header className="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-8 shrink-0">
          <div className="flex items-center gap-4">
            <button 
              ...
              className="p-2.5 hover:bg-slate-50 rounded-xl text-slate-500 transition-all active:scale-95 border border-transparent hover:border-slate-200"
            >
